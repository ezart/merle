...
func (h *Hub) newDevice(id, model, name string, startupTime time.Time) *hubDevice {
	if gen, ok := h.supportedDevices[model]; ok {
		dev := gen(id, model, name, startupTime)
		err := dev.Init(true)
		if err != nil {
			return nil
		}
...
func (h *Hub) getDevice(id string) *hubDevice {
...
func (h *Hub) openDB() error {
	...
	for rows.Next() {
		...
		if d == nil {
			log.Printf("Hub no support for device %s, %s, %s",
				id, model, name)
			continue
		}
